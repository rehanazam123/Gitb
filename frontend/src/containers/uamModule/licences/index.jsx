...
} from '../../../store/features/uamModule/licences/apis';
import { useSelector } from 'react-redux';
import { selectTableData } from '../../../store/features/uamModule/licences/selectors';
...
const Index = () => {
  ...
  const handleExport = (optionType) => {
    if (optionType === 'All Licenses') {
      jsonToExcel(dataSource, 'License');
    } else if (optionType === 'Template') {
      jsonToExcel([generateObject(dataKeys)], 'license_template');
    }
  ...
  return (
    <CustomSpin spinning={isFetchRecordsLoading || isDeleteRecordsLoading}>
      <div>
        {open ? (
          <Modal
        ...
        <DefaultCard sx={{ width: `${width - 105}px` }}>
          <PageHeader pageName="Licenses" buttons={buttons} />
          <DefaultTable
