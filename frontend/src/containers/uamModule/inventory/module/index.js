...
import { fetchDeviceModules } from '../../../../services/devicesServices';
...
const Module = () => {
  ...
  const columns = [
    ...
    {
      title: 'Module Slot',
      dataIndex: 'modules_slot',
      key: 'modules_slot',
      // sorter: (a, b) => a.modules_slot?.localeCompare(b.modules_slot),
      // ...getColumnSearchProps("modules_slot"),
    },
    {
      title: 'Serial Number',
  ...
  //   setLoading(true);
  //   try {
  //     const response = await axios.get(baseUrl + '/device_inventory/modules', {
  //       headers: {
  ...
  const getModule = async () => {
    setLoading(true);
    try {
      const data = await fetchDeviceModules();
      ...
      <DefaultCard sx={{ width: `${width - 105}px` }}>
        ...
        <CustomSpin spinning={loading}>
          <DefaultTable
            ...
            size="small"
            columns={columns}
            dataSource={filteredData?.length > 0 ? filteredData : moduleData}
            onChange={handleChange}
            // rowKey="id"
            style={{ whiteSpace: 'pre' }}
            pagination={{
              defaultPageSize: 10,
              pageSizeOptions: [10, 50, 100, moduleData?.length],
            }}
            rowSelection={false}
            scroll={{
              x: 3000,
