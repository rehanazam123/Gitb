...
from Cisco.ACI.psu_data import  Apic_DevicePowerStackUpdater
# from ACI.testingapic import APIClient
from Cisco.IOS.show_switcs import DevicePowerStackUpdater
...
class DeviceProcessor:
        ...
        with self.db_connection.session_scope() as session:
            ...
            try:
                ...
                devices = (
                    ...
                    # .filter(Device.ip_address=='10.250.100.10')
                    # .filter(DeviceInventory.psu_count==0)
              .all()
                )
                print(len(devices))
                if devices:
    ...
    def process_device(self, vendor, device, device_inventory, devicetype,session):
        ...
        device_handlers = {
            "cisco": {
                "apic": lambda device, device_inventory, session: (
                    self.handle_apic(device, device_inventory, session)
                    if device_role == "controller"
                    else self.log_unhandled_role(device_role, "APIC")
                ),
    ...
    def handle_apic(self, device, device_inventory, session):
        ...
        updater = Apic_DevicePowerStackUpdater(device, device_inventory, password_group, session)
        data=updater.fetch_psu_info(device)


    def handle_mds(self, device, device_inventory, session):
        logging.info(f"Handling Nexus device: {device.ip_address}")
        password_group = self.get_password(device, session)
        if password_group:
            updater = DevicePowerStackUpdater(device, device_inventory, password_group, session)
            success = updater.update_device_inventory()
