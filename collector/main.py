...
from Cisco.mds.power import mds_Power
from Cisco.ios.power import ios_Power
...
class DeviceProcessor:
        ...
        with self.db_connection.session_scope() as session:
            ...
            try:
                ...
                devices = (
                    ...
                    # # .filter(Device.device_type_id==1)
    ...
    def handle_mds(self, device, device_inventory, session):
        ...
        password_group = self.get_password(device, session)
        mds = mds_Power(device, device_inventory, password_group,session).main()
    ...
    def handle_ios(self, device, device_inventory, session):
        ...
        password_group = self.get_password(device, session)
        if password_group:
            ios = ios_Power(device, device_inventory, password_group,session).main()

    def handle_iosxe(self, device, device_inventory, session):
        logging.info(f"Handling IOSXE device: {device.ip_address}")
        password_group = self.get_password(device, session)
        if password_group:
            ios_xe = ios_Power(device, device_inventory, password_group).main()
